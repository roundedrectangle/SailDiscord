...
async def send_channel(c: discord.abc.GuildChannel, myself_id):
    ...
    qsend(f'channel{c.guild.id}', getattr(c.category, 'name', ''),
            ...
            perms.manage_messages, perms.add_reactions,
            getattr(c, 'topic', '') or '',
            *read_args,
    )
...
def send_dms(channel_list: List[discord.DMChannel | discord.GroupChannel | Any], cacher: Cacher, async_runner, send_unread = False):
    # TODO?: update DMs order when last message ID changes
...
def generate_extra_message(message: discord.Message | discord.MessageSnapshot, cacher: Cacher | None = None, emoji_size: Any | None = None, ref={}):
    ...
    if isinstance(message, discord.Message):
        content = emojify(content, cacher, emoji_size, CUSTOM_EMOJI_RE_ESCAPED, 1)
...
def send_guild_emojis(g: discord.Guild, cacher: Cacher):
    qsend(f'serverEmojis{g.id}', [{
        'reactionId': str(e),
        'asset': cacher.easy(e.url, e.id, ImageType.EMOJI)
    } for e in g.emojis if e.is_usable()])