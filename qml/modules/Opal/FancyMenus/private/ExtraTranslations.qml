//@ This file is part of opal-fancymenus.
//@ https://github.com/Pretty-SFOS/opal-fancymenus
//@ SPDX-FileCopyrightText: 2021-2024 Mirian Margiani
//@ SPDX-FileCopyrightText: 2024 roundedrectangle
//@ SPDX-License-Identifier: GPL-3.0-or-later

import QtQuick 2.0

//@ This file MUST NOT be used anywhere. It contains a list of some commonly
//@ needed strings for which this module will provide default translations.
//@ Do not include this file in qmldir. This file may contain invalid QML.

Item {
    //@ commonly used strings
    // readonly property string a: qsTranslate("Opal.FancyMenus.Common", "This is a sentence.")
    // readonly property string b: qsTranslate("Opal.FancyMenus.Common", "Here are %n sentences.", "", 5)
    // readonly property string c: qsTranslate("Opal.FancyMenus.Common", "My hands are typing words.")
}
