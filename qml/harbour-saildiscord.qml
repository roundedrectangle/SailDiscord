...
ApplicationWindow {
    ...
    Python {
        ...
        function init(loggedInHandler, serversModel, dmHandler, dmUpdateHandler, refreshHandler) {
            ...
            setHandler('group', function(channelId, unread, mentions, name, icon, iconBase) { dmHandler({_id: '-1', name: name ? shared.emojify(name) : qsTr("Unnamed", "group"), image: icon, dmChannel: channelId, textSendPermissions: true, iconBase: iconBase ? iconBase : qsTr("Unnamed", "group"), unread: unread, mentions: mentions}) })
